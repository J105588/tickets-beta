...
5. デプロイURLを `config.js` の `GAS_API_URLS` に先頭もしくは追加入力
   - 例: `const GAS_API_URLS = [ 'https://script.google.com/macros/s/NEW_DEPLOY_ID/exec', GAS_API_URL ];`
6. フロントは変更不要。既存の `api.js` がフェイルオーバーを自動実行します
...
## 🔌 オフライン対応 v2（OfflineSyncV2 + Service Worker）

### 目的
- オフライン中の操作（予約・チェックイン・編集）を確実にキューへ保存
- オンライン復帰時に順序を保ってGASへ反映
- 事前に開いたページやアセットをオフラインでも表示

### 構成要素
- `offline-sync-v2.js`: オフライン操作キュー・同期・競合処理
- `sw.js`: Service Worker（静的資産とHTMLをキャッシュ）
- `api.js`: JSONP API。`window.GasAPI`を公開、OfflineSyncV2が検出
...
### 競合とリトライ
- 前提条件（キャッシュのバージョン）不一致時は競合扱い
- 直近5分以内の操作は緩和して送出を試行
- 残存競合は`resolveConflicts()`で最小データ更新→再試行
- リトライは最大3回、バックオフあり

### タイムアウト/安全装置
- `waitForGasAPI()` 10s タイムアウト
- 同期処理 30s タイムアウト（通知＋再試行）
- 連続エラーが多い場合は一時停止してユーザー通知

### Service Worker の動作
- キャッシュ名: `tickets-static-v3`
- プリキャッシュ: 主要HTML/JS/CSS/`offline-sync-v2.*`
- ナビゲーションはキャッシュ優先（ignoreSearch）
- 初回未キャッシュはネットワーク→取得成功時に保存、失敗時フォールバック
- `skipWaiting()`/`clients.claim()`で即時有効化

### 導入/更新手順
1) `sw.js` を配置（済）
2) `seats.html` などでService Worker登録を有効化（未導入の場合）
```html
<script>
...
```
3) 初回はオンラインで各ページ（index/timeslot/seats/walkin）を一度開く
4) 以降、オフラインでもこれらページは表示可能

### デバッグのヒント
- `OfflineSyncV2.showQueueStatus()` でキュー確認
- コンソールログに同期の進捗（開始/成功/競合/再試行）を出力
- GAS反映確認はスプレッドシートのC列/D列/E列を参照
...
### 1) オフラインバッジ位置の変更
- `offline-sync-v2.css` の `.offline-indicator` を画面右下（bottom: 10px; right: 10px）へ移動

### 2) オフライン時の座席データ表示（オフラインファースト）
- `seats-main.js`
  - 初期表示/手動更新/自動更新の各場面で、オフライン時はサーバーへ取りに行かずローカルキャッシュからマップを描画
  - オンライン時は取得成功後にキャッシュへ書き込み（再訪に備える）

### 3) 全ページにオフライン機能を適用
- `timeslot.html` / `walkin.html`
  - オフラインバッジと同期進捗バーを追加
  - `offline-sync-v2.css` と `offline-sync-v2.js` を追加読み込み
- `timeslot-main.js` / `walkin-main.js`
  - `initializeOfflineIndicator()` を実装し、状態の反映と同期進捗の可視化
- `index.html`
  - `offline-sync.js` から `offline-sync-v2.js` に切替

### 4) 当日券（walkin）操作のオフライン対応
- `walkin-main.js`
  - GAS からの `offline_delegate` 応答を受け取り、`OfflineSyncV2` に操作をキュー追加
  - オフラインでも「受け付けました」を即時通知し、オンライン復帰時に自動同期

### 5) どのページでも同期できるフローティングウィジェット
- `offline-sync-v2.js`
  - `injectFloatingSyncWidget()` を追加
  - 右下に小さなウィジェットを常時表示し、状態（オンライン/オフライン/同期中）とキュー件数を表示
  - 「今すぐ同期」「詳細」ボタンでどのページからでも同期実行・キュー内容確認
  - 軽量実装（ポーリングのみ、ネットワーク負荷なし）

### 6) Service Worker によるページ遷移のオフライン対応
- `sw.js`
  - 主要 HTML と静的資産をキャッシュ
  - ナビゲーションはキャッシュ優先で提供し、初回未キャッシュ時のみネットワーク→保存

### 7) ドキュメント/コミット設定
- `.gitignore` から `README.md` の除外を削除（コミット可能に）
- 本セクションに今回の変更点を追記
