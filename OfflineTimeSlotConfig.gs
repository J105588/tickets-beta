// ==== Offline 用 時間帯設定管理ファイル ====
// 本体の TimeSlotConfig.gs を独立プロジェクト用に複製
...
function _getAllTimeslotsForGroup(group) {
  ...
  for (const day in groupSchedule) {
    ...
    for (const timeslot in daySchedule) {
      ...
      results.push({ day: day, timeslot: timeslot, time: time, displayName: `${timeslot}時間帯 (${time})` });
    }
  }
  return results;
}

function getDayName(day) { return day == 1 ? '1日目' : '2日目'; }
